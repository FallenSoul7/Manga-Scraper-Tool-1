...
function ExtensionAvatar({ ext, size = 48 }: { ext: { name: string; iconUrl: string | null }; size?: number }) {
  ...
  if (ext.iconUrl && !errored) {
    return (
      <img
        ...
        className="rounded-md bg-muted shrink-0 object-cover"
        style={{ width: size, height: size }}
      />
    );
  }
  // Fallback with initials
  const initials = ext.name
    .split(/\s+/)
    .map((w) => w[0])
    .join("")
    .slice(0, 2)
  ...
  return (
    <div
      className="rounded-md bg-muted text-muted-foreground flex items-center justify-center shrink-0 font-semibold"
  ...
  return (
    <main className="container mx-auto px-4 py-6 sm:py-8 max-w-5xl">
      <div className="mb-6 sm:mb-8">
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-serif font-bold mb-2 sm:mb-4">
          Sources
        </h1>
        <p className="text-muted-foreground text-base sm:text-lg">
          Manage and switch between manga sources. The selected source powers
            ...
            <div className="flex items-center gap-2 shrink-0" onClick={(e) => e.stopPropagation()}>
              {src.id !== "en.comix" && (
                <Button
                  ...
                  aria-label="Remove"
                  onClick={() => {
                    storeActions.uninstallSource(src.id);
                    toast({ title: `Removed ${src.name}` });
                  }}
                >
...
function BrowseTab({
  ...
  }, [search, lang, showNsfw, supportedOnly]);
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {ext.isNsfw && (
                    <span className="text-[10px] px-1.5 py-0 rounded bg-rose-500/10 text-rose-600 dark:text-rose-400">
                      18+
                    </span>
                  )}
                  {!ext.supported && (
                ...
                <p className="text-xs text-muted-foreground truncate">
                  {langLabel(ext.lang)} · v{ext.versionCode}
                </p>
              </div>
              {installed ? (
                ...
                <Button
                  size="sm"
                  variant={ext.supported ? "default" : "outline"}
                  disabled={!ext.supported}
                  onClick={() => {
                    storeActions.installSource({
                      id: ext.id,
                      name: ext.name,
                      lang: ext.lang,
                      isNsfw: ext.isNsfw,
                    ...
                    toast({
                      title: `Added ${ext.name}`,
                      description:
                        "Open the Added tab and pick 'Select this extension' to switch to it.",
                    });
                  }}
                >
