Only 3 things change in reader.tsx — import the helper, read the setting, and wrap image URLs. Nothing else touched:
...
3 changes only:
  ...
  •	Added import { getProxiedImageUrl } from "@/lib/vpn"
  •	Added const vpnEnabled + resolveImageUrl helper near the top of the component
  •	Both <img src=...> calls (main pages + appended chapters) now use resolveImageUrl(page.url) instead of proxyImage(...)import express, { type Express } from "express";
